...
abstract class Action
{
    ...
    public static function getAction($actionClassName, $initObject = null)
    {
        if (!is_string($actionClassName)
            || !preg_match('/^[a-zA-Z_][a-zA-Z0-9_]*$/i', $actionClassName)) {
            $errmsg = 'action class name invalid: actionClassName['
        ...
        if (!($actionObject instanceof Action)) {
            $errmsg = 'The reflected object is not an Action class: actionClassName['
                    . $actionClassName . ']';
            trigger_error($errmsg, E_USER_ERROR);
            return null;
        }
